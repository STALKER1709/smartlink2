            ...
            <div class="lg:col-span-2">
                <div class="bg-surface-container-lowest rounded-xl border border-outline-variant p-6">
                    <div class="flex items-center gap-4">
                        <div class="h-20 w-20 rounded-full bg-surface-container flex items-center justify-center overflow-hidden shrink-0 border border-outline-variant">
                            ...
                            @else
                                <span class="text-2xl font-semibold text-on-surface-variant">{{ Str::substr($providerProfile->business_name, 0, 1) }}</span>
                            @endif
                        </div>
                        ...
                        <div>
                            <div class="flex items-center gap-2">
                                ...
                                @if ($providerProfile->is_verified)
                                    <span class="text-primary" title="Prestataire vérifié">
                                        <span class="material-symbols-outlined" style="font-variation-settings: 'FILL' 1;">verified</span>
                                    </span>
                                @endif
                                <x-promoted-badge :profile="$providerProfile" />
                            </div>
                            <p class="text-sm text-on-surface-variant">{{ $providerProfile->category?->name }} @if ($providerProfile->city) · {{ $providerProfile->city }} @endif</p>
                    ...
                    @if ($providerProfile->description)
                        <p class="mt-4 text-on-surface whitespace-pre-line">{{ $providerProfile->description }}</p>
                    @endif

                    @if ($providerProfile->address || $providerProfile->quarter)
                        <p class="mt-4 text-sm text-on-surface-variant flex items-center gap-1">
                    ...
                    @if ($providerProfile->whatsappUrl())
                        <a href="{{ $providerProfile->whatsappUrl() }}" target="_blank" rel="noopener"
                           class="mt-3 inline-flex items-center gap-2 rounded-full bg-secondary px-4 py-2 text-sm font-button-text font-semibold text-on-secondary hover:opacity-90 transition-opacity">
                    ...
                    @if ($providerProfile->latitude && $providerProfile->longitude)
                        <div class="mt-4">
                            <div id="provider-map-show" class="w-full h-48 rounded-lg border border-outline-variant z-0"></div>
                        </div>
                    @endif
                        ...
                        <div class="mt-4">
                            ...
                            <div class="mt-2 flex flex-wrap gap-2">
                                @foreach ($providerProfile->service_areas as $area)
                                    <span class="inline-flex items-center rounded-full bg-surface-container px-2.5 py-0.5 text-xs text-on-surface-variant">{{ $area }}</span>
                                @endforeach
                            </div>
                        </div>
                    @endif
                    ...
                    @if (! empty($providerProfile->opening_hours))
                        <div class="mt-4">
                            <h3 class="text-sm font-semibold text-on-surface">Horaires d'ouverture</h3>
                            <ul class="mt-2 text-sm text-on-surface-variant space-y-1">
                ...
                <!-- Services -->
                <div class="mt-8">
                    <h2 class="font-headline-md text-headline-md text-on-surface mb-4">Services proposés</h2>

                    @if ($services->isEmpty())
                        <p class="text-on-surface-variant">Ce prestataire n'a pas encore publié de service.</p>
                    @else
                ...
                <!-- Reviews -->
                <div class="mt-8">
                    <h2 class="font-headline-md text-headline-md text-on-surface mb-4">Avis clients</h2>
                        ...
                        <div class="space-y-4">
                            @foreach ($reviews as $review)
                                <div class="bg-surface-container-lowest rounded-xl border border-outline-variant p-4">
                                    ...
                                    @if ($review->comment)
                                        <p class="mt-2 text-sm text-on-surface">{{ $review->comment }}</p>
                                    @endif
                                    <p class="mt-1 text-xs text-on-surface-variant">{{ $review->created_at->format('d/m/Y') }}</p>
                                </div>
                            @endforeach
                        </div>
                    @endif
