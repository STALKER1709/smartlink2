    ...
    <div class="flex-1 overflow-y-auto p-4 space-y-3">
        @forelse ($conversation->messages as $message)
            <div class="flex {{ $message->sender_id === Auth::id() ? 'justify-end' : 'justify-start' }}">
                <div class="max-w-[75%] rounded-lg px-3 py-2 text-sm {{ $message->sender_id === Auth::id() ? 'bg-indigo-600 text-white' : 'bg-gray-100 text-gray-800' }}">
                    <p class="whitespace-pre-line">{{ $message->body }}</p>
                    <p class="mt-1 text-[10px] {{ $message->sender_id === Auth::id() ? 'text-indigo-200' : 'text-gray-400' }}">
