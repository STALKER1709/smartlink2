@auth
    @if (auth()->user()->isProvider())
        ...
        @if (! $subscription)
            <div class="bg-error-container border-b border-outline-variant">
                <div class="max-w-container mx-auto px-margin-mobile md:px-margin-tablet lg:px-margin-desktop py-3 flex flex-wrap items-center justify-between gap-3">
                    ...
                    <a href="{{ route('provider.subscription.show') }}" class="shrink-0 rounded-full bg-error px-4 py-2 text-label-md font-button-text font-semibold text-on-error hover:opacity-90 transition-opacity">
                        {{ __('ui.subscription.renew') }}
                    </a>
                </div>
            </div>
        @elseif ($subscription->daysRemaining() <= 7)
            <div class="bg-secondary-container/20 border-b border-outline-variant">
                <div class="max-w-container mx-auto px-margin-mobile md:px-margin-tablet lg:px-margin-desktop py-3 flex flex-wrap items-center justify-between gap-3">
                    <p class="text-label-md font-medium text-secondary">
                    ...
                    <a href="{{ route('provider.subscription.show') }}" class="shrink-0 rounded-full bg-secondary px-4 py-2 text-label-md font-button-text font-semibold text-on-secondary hover:opacity-90 transition-opacity">
        ...
        @elseif ($remaining !== null && $remaining === 0)
            <div class="bg-secondary-container/20 border-b border-outline-variant">
                <div class="max-w-container mx-auto px-margin-mobile md:px-margin-tablet lg:px-margin-desktop py-3 flex flex-wrap items-center justify-between gap-3">
                    <div class="text-label-md text-secondary">
