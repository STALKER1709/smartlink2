    ...
    @if (auth()->user()->isProvider())
        ...
        @if (! $subscription)
            <div class="bg-error-container border-b border-outline-variant">
                <div class="max-w-container mx-auto px-margin-mobile md:px-margin-desktop py-3 flex flex-wrap items-center justify-between gap-3">
                    <div class="text-label-lg text-on-error-container">
                        <p class="font-semibold">{{ __('ui.subscription.expired') }}</p>
                        <p>{{ __('ui.subscription.expired_hint') }}</p>
                    </div>
                    <a href="{{ route('provider.subscription.show') }}" class="shrink-0 rounded-full bg-error px-4 py-2 text-label-lg font-button-text font-semibold text-on-error hover:opacity-90 transition-opacity">
        ...
        @elseif ($subscription->daysRemaining() <= 7)
            <div class="bg-tertiary-container/15 border-b border-outline-variant">
                <div class="max-w-container mx-auto px-margin-mobile md:px-margin-desktop py-3 flex flex-wrap items-center justify-between gap-3">
                    <p class="text-label-lg font-medium text-tertiary">
                    ...
                    <a href="{{ route('provider.subscription.show') }}" class="shrink-0 rounded-full bg-tertiary px-4 py-2 text-label-lg font-button-text font-semibold text-on-tertiary hover:opacity-90 transition-opacity">
        ...
        @elseif ($remaining !== null && $remaining === 0)
            <div class="bg-tertiary-container/15 border-b border-outline-variant">
                <div class="max-w-container mx-auto px-margin-mobile md:px-margin-desktop py-3 flex flex-wrap items-center justify-between gap-3">
                    <div class="text-label-lg text-tertiary">
