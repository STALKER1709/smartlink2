...
@php
    ...
    $classes = 'inline-flex w-full items-center justify-center gap-2 rounded-full bg-primary font-button-text font-semibold text-on-primary transition-colors hover:bg-primary-container '
        .($compact ? 'px-5 py-2.5 text-label-md' : 'px-4 py-3');
@endphp
...
@auth
    ...
    @else
        <p class="text-center text-label-md text-on-surface-variant">{{ __("Seuls les clients peuvent envoyer une demande.") }}</p>
    @endcan
@else
    ...
    @unless ($compact)
        <p class="mt-2 text-center text-label-sm text-on-surface-variant">
