<x-app-layout>
    ...
    <div class="max-w-xl mx-auto px-margin-mobile md:px-margin-desktop py-8">
        <form action="{{ route('admin.plans.update', $plan) }}" method="POST" class="bg-surface-container-lowest rounded-xl border border-outline-variant p-6 space-y-5">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <p class="mt-1 text-xs text-on-surface-variant">{{ __('ui.admin_plans.unlimited_hint') }}</p>
            ...
            <div class="flex items-center justify-between gap-3 border-t border-outline-variant pt-5">
                <a href="{{ route('admin.plans.index') }}" class="text-sm text-on-surface-variant hover:text-on-surface">
                    {{ __('ui.cancel') }}
                </a>
