...
<nav x-data="{ open: false }" class="border-b border-outline-variant bg-surface-container-lowest">
    <div class="mx-auto max-w-container px-margin-mobile md:px-margin-desktop">
        <div class="flex h-16 justify-between gap-4">
            ...
            <div class="hidden shrink-0 items-center gap-2 sm:ms-6 sm:flex">
                <div class="flex overflow-hidden rounded-full border border-outline-variant text-label-md font-semibold">
                ...
                @auth
                    <a href="{{ route('notifications.index') }}"
                       ...
                       title="{{ __('Notifications') }}">
                        ...
                        @if ($nonLues > 0)
                            <span class="absolute right-0.5 top-0.5 inline-flex h-4 w-4 items-center justify-center rounded-full bg-error font-label-md text-label-md font-bold text-on-error">
                                {{ $nonLues > 9 ? '9+' : $nonLues }}
                            </span>
                        @endif
                    </a>
                    ...
                    <x-dropdown align="right" width="56">
                        <x-slot name="trigger">
                            <button class="inline-flex max-w-[10rem] items-center gap-1 rounded-full border border-transparent bg-surface-container-lowest px-3 py-2 text-label-lg font-medium leading-4 text-on-surface-variant transition duration-150 ease-in-out hover:bg-surface-container-low hover:text-on-surface focus:outline-none">
                        ...
                        <x-slot name="content">
                            <div class="border-b border-outline-variant px-4 py-3">
                                <p class="truncate text-label-lg font-semibold text-on-surface">{{ $utilisateur->name }}</p>
                                <p class="truncate text-label-md text-on-surface-variant">{{ $utilisateur->email }}</p>
                            </div>
                ...
                @else
                    <a href="{{ route('login') }}" class="px-2 text-label-lg font-medium text-on-surface-variant hover:text-on-surface">{{ __('Se connecter') }}</a>
                    <a href="{{ route('register') }}" class="rounded-full bg-primary px-5 py-2 font-button-text text-label-lg font-semibold text-on-primary transition-colors hover:bg-primary-container">{{ __("S'inscrire") }}</a>
                @endauth
            </div>
    ...
    <div x-show="open" x-cloak class="sm:hidden">
        ...
        <div class="border-t border-outline-variant pb-1 pt-4">
            @auth
                <div class="px-4">
                    ...
                    <div class="text-label-lg font-medium text-on-surface-variant">{{ $utilisateur->email }}</div>
                </div>
        ...
        <div class="flex gap-2 border-t border-outline-variant px-4 py-3">
            <a href="{{ route('locale.switch', 'fr') }}"
               class="rounded-full px-3 py-1.5 text-label-lg font-medium {{ $locale === 'fr' ? 'bg-primary text-on-primary' : 'text-on-surface-variant hover:bg-surface-container-low' }}">{{ __("Français") }}</a>
            <a href="{{ route('locale.switch', 'en') }}"
               class="rounded-full px-3 py-1.5 text-label-lg font-medium {{ $locale === 'en' ? 'bg-primary text-on-primary' : 'text-on-surface-variant hover:bg-surface-container-low' }}">{{ __("English") }}</a>
        </div>
    </div>
</nav>
